...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        self.a_levels.append(set()) # A_level

        for action in self.all_actions:
            a_node = PgNode_a(action)
            # check if action's prenodes is a subset of the previous S level
            if a_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(a_node)

                # we now need to update link parent-child
                for node in self.s_levels[level]:
                    if node in a_node.prenodes:
                        node.children.add(a_node)
                        a_node.parents.add(node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check effect node_a1 negates precond of node_a2
        ...
        # check effect node_a2 negates precond of node_a1
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # we need to check for mutex between node_a1's parents vs node_a2's parents
